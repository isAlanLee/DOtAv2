...
usage() {
  cat <<'EOF'
Usage: bash scripts/resume_from_mbe_score.sh [options]

Resume the DOtA pipeline from MBE box scoring:
  04 score_mbe_boxes -> 05 prepare_dota_hypes -> 06 train_with_pseudo_labels -> 07 test_final_model
...
Options:
  ...
  --final-checkpoint-dir PATH    Existing final checkpoint; skips pseudo-label training.
...
RUN_ID="resume_mbe_score_$(date +%Y%m%d_%H%M%S)"
...
log_summary "Resume from MBE score. Shutdown mode: $([[ "$DRY_RUN" -eq 1 || "$NO_SYSTEM_SHUTDOWN" -eq 1 ]] && echo 'disabled' || echo "$SHUTDOWN_COMMAND")"
...
GENERATED_DOTA_HYPES="$RUN_DIR/generated_hypes/point_pillar_intermediate_fusion_dota.resume_mbe_score.yaml"
...
log_summary "RESUME PIPELINE COMPLETE. Logs are in $RUN_DIR"
