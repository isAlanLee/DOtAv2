...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate initial pseudo-label recall/AP against GT.")
    ...
    parser.add_argument("--score-thresholds", default="0.01,0.02,0.05,0.1,0.2")
...
def final_recall(result_stat, iou):
    ...
    gt = stat["gt"]
    if gt == 0 or len(stat["tp"]) == 0:
        return 0.0
    return float(np.sum(stat["tp"])) / float(gt)
...
def main():
    ...
    box_dir = os.path.join(args.pseudo_root, "pre_box_test_full")
    score_dir = os.path.join(args.pseudo_root, "pre_score_test_full")
    ...
    for idx in tqdm(indices):
        ...
        for thr in thresholds:
            ...
            if np.sum(keep) == 0:
                ...
                det_corners = box_utils.boxes_to_corners_3d(boxes[keep], order="lwh")
    ...
    for thr in thresholds:
        ...
        for iou in (0.30, 0.50, 0.70):
            ap, mrec, mpre = eval_utils.calculate_ap(stats[thr], iou, True)
