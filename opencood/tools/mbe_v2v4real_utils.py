...
def remove_ground_points(point_cloud, max_iterations=100,
    ...
    for _ in range(max_iterations):
        ...
        distances = np.abs(
            model.predict(point_cloud[:, :2]) - point_cloud[:, 2])
...
def classify_mbe_state(inter_points_number_total, convex_hull_number_total,
                       distance_total):
...
def mbe_box_filter(pseudo_labels_world, multi_agent_point, cav_positions,
                   frame_idx):
    ...
    for box in pseudo_labels_world:
        ...
        keep.append(
            classify_mbe_state(inter_points_number_total,
                               convex_hull_number_total,
                               distance_total) == 1)
