...
class IntermediateFusionDataset(basedataset.BaseDataset):
    ...
    def __init__(self, params, visualize, train=True):
        ...
        self.label_free_training = params['lable_free']
        self.iterative_training = params['iterative_training']

        if not self.train:
            self.label_free_training = False
            self.iterative_training = False

        if self.iterative_training:
            self.pseudo_lable_path = params['pseudo_lable_path']
    ...
    def __getitem__(self, idx):
        ...
        if self.iterative_training:
            ...
            object_stack = np.load(self.pseudo_lable_path + f'/out_pseduo_labels_with_score_v4_{idx}.npy')
            ...
            object_stack_noise = np.load(self.pseudo_lable_path + f'/out_pseduo_labels_noise_with_score_v4_{idx}.npy')
            for arr_noise in object_stack_noise:
                t_noise = arr_noise[3]
                arr_noise[3] = arr_noise[5].copy()
                arr_noise[5] = t_noise

            object_bbx_center = \
                np.zeros((self.params['postprocess']['max_num'], object_stack.shape[1])) #xqm 7/8
            object_bbx_center_noise = \
                np.zeros((self.params['postprocess']['max_num']*10, object_stack_noise.shape[1])) #xqm 7/8
        ...
        if self.iterative_training:
            object_bbx_center_noise[:object_stack_noise.shape[0], :] = object_stack_noise
    ...
    def collate_batch_train(self, batch):
        ...
        for i in range(len(batch)):
            ...
            if self.iterative_training:
                object_bbx_center_noise.append(ego_dict['object_bbx_center_noise'])
        ...
        if self.iterative_training:
            object_bbx_center_noise = torch.from_numpy(np.array(object_bbx_center_noise))
